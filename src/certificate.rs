...
impl<'a> Renewal<'a> {
    ...
    pub fn load_old_cert(&mut self, pem: &'a Pem) -> Result<(), String> {
        let (_, old_cert) = x509_parser::parse_x509_certificate(pem.contents.as_bytes())
    ...
    pub fn generate_key_pair(&mut self) -> Result<(), String> {
        let private_key_keypair = match self.signature_algorithm {
            Rsa(bits) => {
                ...
                let key =
                    rsa::RsaPrivateKey::new(&mut rng, bits.bit_size).map_err(|e| e.to_string())?;
                let key_pem = key
    ...
    pub fn generate_signing_request(&mut self) -> Result<(), String> {
        //-> Result<CertificateSigningRequest, String> {
        let mut params = rcgen::CertificateParams::default();

        if let Some(old_cert) = self.current_cert.as_ref() {
            if let Some(san) = old_cert
                .subject_alternative_name()
                .map_err(|e| e.to_string())?
            {
                let san_type_list: Result<Vec<SanType>, String> = san
                    .value
                    .general_names
                    .iter()
                    .cloned()
                    .map(|item| -> Result<SanType, String> {
                        match item {
                            GeneralName::RFC822Name(s) => Ok(SanType::Rfc822Name(s.to_string())),
                            GeneralName::DNSName(s) => Ok(SanType::DnsName(s.to_string())),
                            GeneralName::URI(u) => Ok(SanType::URI(u.to_string())),
                            GeneralName::IPAddress(bytes) => {
                                let octets: Vec<String> =
                                    bytes.iter().map(|byte| byte.to_string()).collect();

                                let ipaddr = IpAddr::from_str(octets.join(".").as_str())
                                    .map_err(|e| e.to_string())?;
                                Ok(SanType::IpAddress(ipaddr))
                            }
                            // best we can do is convert to string
                            x => Ok(SanType::Rfc822Name(x.to_string())),
                        }
                    })
                    .collect();
            ...
            if let Some(cn) = old_cert.subject.iter_common_name().next() {
                dn.push(
                    DnType::CommonName,
                    DnValue::Utf8String(cn.as_str().map_err(|e| e.to_string())?.to_string()),
                )
            }
            ...
            if let Some(org) = old_cert.subject.iter_organization().next() {
                dn.push(
                    ...
                    DnValue::Utf8String(org.as_str().map_err(|e| e.to_string())?.to_string()),
                );
            }
            ...
            if let Some(org_unit) = old_cert.subject.iter_organizational_unit().next() {
                ...
                dn.push(
                    ...
                    DnValue::Utf8String(org_unit.as_str().map_err(|e| e.to_string())?.to_string()),
                );
            }
            ...
            if let Some(country) = old_cert.subject.iter_country().next() {
                ...
                dn.push(
                    DnType::CountryName,
                    DnValue::Utf8String(country.as_str().map_err(|e| e.to_string())?.to_string()),
                );
            }
            ...
            if let Some(locality) = old_cert.subject.iter_locality().next() {
                ...
                dn.push(
                    DnType::LocalityName,
                    DnValue::Utf8String(locality.as_str().map_err(|e| e.to_string())?.to_string()),
                );
            }
        ...
        let csr = rcgen::CertificateSigningRequest::from_pem(csr_pem.as_str())
            .map_err(|e| e.to_string())?;
    ...
    pub fn renew<U: IntoUrl>(&mut self, url: U) -> Result<String, String> {
        let new_cert_pem = http::request_client_certificate(self, url)?;

        let pem =
            x509_parser::pem::parse_x509_pem(new_cert_pem.as_bytes()).map_err(|e| e.to_string())?;
        let renewed_cert = x509_parser::parse_x509_certificate(pem.1.contents.as_slice())
            .map_err(|e| e.to_string())?;
...
mod tests {
    ...
    fn test_default_cert() {
        ...
        assert_eq!(
            cert.signature_algorithm.to_string(),
            Rsa(&RSA3072).to_string()
        )
    }
