...
pub fn renew_once(renew_cert: &RenewCommand) -> Result<String, String> {
    let key_type = match renew_cert.key_type {
        PrivateKeyType::Rsa => match renew_cert.rsa_key_length.as_ref().unwrap() {
...
pub fn auto_renew(renewal_config: &AutorenewCommand) -> Result<(), String> {
    ...
    let file = fs::File::open(renewal_config.configuration_file_path.as_path())
...
fn renew_all_certs(config: &AutorenewalConfig) {
    for certificate in config.certificates.as_slice() {
        let key_type = match config.key_type {
            PrivateKeyType::Rsa => match config.rsa_key_length {
                RsaKeyLengthOption::Rsa2048 => SigAlg::Rsa(&certificate::RSA2048),
                RsaKeyLengthOption::Rsa3072 => SigAlg::Rsa(&certificate::RSA3072),
                RsaKeyLengthOption::Rsa4096 => SigAlg::Rsa(&certificate::RSA4096),
            },
...
fn renew(
    ...
    let (pem_bytes, raw_pem) =
        certificate::pem_to_der_bytes(&conf.certificate_path).unwrap_or_else(|_| exit(2));
    ...
    if force
        || ccx
            ...
            .time_to_expiration()
            .unwrap()
            <= Duration::from_secs(60 * 60 * 24 * conf.renew_grace_period)
    {
        ccx.current_cert_pem = raw_pem;
        ccx.load_privatekey(conf.private_key_path.to_path_buf())
            .unwrap();
        ...
        fs::write(
            conf.private_key_path.to_path_buf(),
            ccx.new_priv_key.unwrap().serialize_pem(),
        )
        .unwrap();

        Ok(format!(
            "Succesfully renewed certificate: {}",
            conf.certificate_path.to_str().unwrap()
        ))
    } else {
